...
R can connect directly to databases. At time of writing this only works for ADW but you should consult the CoDE/KAI IT Team guidance for the latest recommendations for connecting to databases.
...
connection <- "Driver={SQL Server};server=IRA____,Database=db_name;trusted_connection=true;"
...
```
where `IRA___` should be replaces with the server name, and `db_name` by the database name.
...
Once you have accessed your structured data you will have either read it directly into an R dataframe or established a connection to a database. Next you will want to explore the data and transform it.
...
df_out <-
  ...
  filter(column2 == 2)
```
And `dplyr` also contains commands for joining data, grouping and summarising data, sorting data and many others.
