...
Examples of sampling the `df` data frame:
...
# If index larger than the number of rows, cycle round the data frame
indx2 <-
  indx %>%
  map_dbl(~ (.x -1) %% rows + 1 )
...
Note, the `sample_n` command might not work with database connections.
