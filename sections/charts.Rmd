...
There are a number of packages available for network visualization in R. Which package is most suitable will depend on your requirements; are you seeking to visualise a static network, a interactive network, or wanting to perform network analysis? For non-interactive graphs `ggraph` is a good option, and for interactive graphs `visNetwork` is a good.
...
*And a smiliar non-interactive version could be created with this code.*
```{r eval=FALSE}
library(igraph)
library(ggraph)
