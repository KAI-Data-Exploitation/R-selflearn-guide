...
R Markdown can be used to create HTML, PDF, word documents and various presentation formats. However, at the time of writing only HTML documents (or HTML slides) can be produced on our network. But HTML documents have the advantage that they can include interactive elements from packages such as `leaflet` for maps or `plotly` for graphs that produce HTML output.
