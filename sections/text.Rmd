...
Before text can be used it needs to be preprocessed, which usually means cleaning the text and converting words into their root froms either by stemming or lemmatisation.
...
- `tm_map(corpus, removeWords, all_stop)` will remove a list of words from the data you wish to exclude. R already has built in lists of common words such as 'i', 'me', 'you', 'he'. In this case we have used a personalised list called 'all_stop'.  
- `tm_map(corpus, stripWhitespace)` will remove any spacing that has occurred when removing number, punctuation, etc.  
...
clean_corpus <- function(corpus){
  ...
  tm_map(stripWhitespace) -> clean_corpus
  return(clean_corpus)
}
```

Which can then be applied on a corpus.
```{r eval=FALSE}
...
Stemming is the process of reducing words to their stem, base or root form. This is done to group together similar words so their frequencies reflect the true use of the word, another more sophisticated version of this is lemmatisation.

Stemming just removes the end of the word to approximate the root and can be dobe in `tm` using the following command.
```{r, eval=FALSE}
...
Common visualisations used to explore text data include word frequency charts and word clouds. These are outlined below, where the *crude* dataset (which loads a corpus of documents about crude oil) provided by the `tm`.
...
After cleaning your data you'll have to create a document term matrix using `tm`. The code below will do this.  
...
Another way you can visualise your data is using a word cloud. Word clouds do have their limitations but are good for picking out words at a quick glance. A package you can use to create these is `wordcloud2`. Below we create a word cloud of all words appearing more than two times in the corpus. Note, the word frequencies need to be sorted so that the most commonly occuring words appear at the start of the data frame.
```{r eval=FALSE, echo=TRUE}
library(wordcloud2)

words_filtered <- word_counts %>% filter(freq > 2) %>% arrange(desc(freq))
wordcloud2(words_filtered) 
```

```{r echo=FALSE}
# Saved wordcloud used to avoid package conflicts with
# leaflet, plotly and networkVis

shiny::tags$iframe(src = "images/wordcloud.html", 
                       width=700, height=500)
```

This produces interactive HTML output, if you require an image file you can click on *Export > Save As Image* directly above the word cloud (in the Viewer pane of RStudio). More information about the `wordcloud2` package can be found [here](https://cran.r-project.org/web/packages/wordcloud2/vignettes/wordcloud.html).

## Machine learning

Machine learning can be used to try and detect patterns amongst text documents (*topic modelling*) and text can also be used as features in predictive models.
...
For other types of machine learning you will want to convert the document term matrix into a data frame of features. However, the document term matrix may contain features that do not appear very often which you want to remove, this can be done with the `removeSparseTerms()` function before converting to a dataframe.
...
- In the previous sections we only computed unigrams (single words). Often you will want to include bigrams (two adjacent words) or trigrams in your analysis. `RWeka` is commonly used to create ngrams in R but the available version is currently not working on our network. [This post](https://datascience.stackexchange.com/questions/18522/text-mining-in-r-without-rweka) discusses how to create ngrams in R without using `RWeka`.
- When the document-term-matrix is created you can specify where to weight the values via their term frequency-inverse document frequency (*tf-idf*).
